...
@section('content')
    <h1 class="text-2xl font-bold mb-6">Edit Pengembalian</h1>

    <div class="bg-white shadow rounded-xl p-6 w-full md:w-2/3 lg:w-1/2">
        <form action="{{ route('pengembalian.update', $pengembalian->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="nama_pengembali" 
                       value="{{ old('nama_pengembali', $pengembalian->nama_pengembali) }}"
                ...
                <select name="kode_barang" id="kode_barang"
                    ...
                    @foreach($peminjaman as $item)
                        <option value="{{ $item->kode_barang }}"
                                ...
                                {{ old('kode_barang', $pengembalian->kode_barang) == $item->kode_barang ? 'selected' : '' }}>
                            {{ $item->kode_barang }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" name="nama_barang" id="nama_barang"
                       value="{{ old('nama_barang', $pengembalian->nama_barang) }}"
            ...
            <div>
                ...
                <input type="number" name="jumlah" id="jumlah"
                       value="{{ old('jumlah', $pengembalian->jumlah) }}"
                       class="w-full px-4 py-2 border rounded-lg bg-gray-100 cursor-not-allowed"
                       readonly>
            </div>
    ...
    <script>
        ...
        document.getElementById('kode_barang').addEventListener('change', updateFields);

        // Set default value saat load edit page
