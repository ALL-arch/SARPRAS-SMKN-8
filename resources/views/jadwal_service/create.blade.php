...
@section('content')
<h1 class="text-2xl font-bold mb-6">Tambah Jadwal Service</h1>
    ...
    <form action="{{ route('jadwal_service.store') }}" method="POST" class="space-y-4">
            ...
            <select name="kode_barang" id="kode_barang"
                ...
                @foreach($inventaris as $inv)
                    <option value="{{ $inv->kode_barang }}" data-nama="{{ $inv->nama_barang }}"
                        {{ old('kode_barang') == $inv->kode_barang ? 'selected' : '' }}>
                        {{ $inv->kode_barang }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <textarea name="keterangan_service" rows="3"
                      class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-yellow-400 focus:outline-none">{{ old('keterangan_service') }}</textarea>
        </div>
